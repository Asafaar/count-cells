...
def detect_and_highlight_empty_areas(image_path, model_path, color=(0, 0, 255)): # color = red
    ...
    mask = np.ones((h, w), dtype=np.uint8) * 255  # יצירת מסיכה לבנה

    for box in results[0].boxes:
        xyxy = box.xyxy[0].int().tolist()
        x1, y1, x2, y2 = xyxy
        cv2.rectangle(mask, (x1, y1), (x2, y2), (0, 0, 0), -1)  # צביעת התיבות בשחור

    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    for contour in contours:
        cv2.fillPoly(img, [contour], color) #צביעת האזורים הריקים בצבע
